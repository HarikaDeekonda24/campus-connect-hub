...
interface AttendanceRequest { id: string; student_name: string; roll_number: string; branch: string; department: string; event_id?: string | null; proof: string; status: string; created_at: string; }
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="campus-gradient rounded-2xl p-6 md:p-8 text-primary-foreground relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-wrap gap-2 mt-3">{userBranches.map(b => <span key={b} className="px-2 py-0.5 rounded bg-primary-foreground/20 text-primary-foreground text-xs font-medium">{b}</span>)}</div>
        </div>
      </motion.div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-display text-foreground flex items-center gap-2">
            <Bell className="w-5 h-5 text-warning" />Pending Attendance
            {pending.length > 0 && <span className="px-2 py-0.5 rounded-full bg-warning/15 text-warning text-xs font-semibold">{pending.length}</span>}
          </h2>
        </div>
        {loading ? <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="campus-card p-4 animate-pulse flex items-center gap-3"><div className="w-10 h-10 bg-muted rounded-lg" /><div className="flex-1 space-y-2"><div className="h-3 bg-muted rounded w-1/2" /><div className="h-3 bg-muted rounded w-1/3" /></div></div>)}</div>
        : pending.length === 0 ? <div className="campus-card p-8 text-center text-muted-foreground"><CheckCircle className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">No pending requests for your branches.</p></div>
        : <div className="space-y-3">{pending.map((req, i) => {
            ...
            <motion.div key={req.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="campus-card p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-lg bg-warning/10 flex items-center justify-center flex-shrink-0 mt-0.5"><span className="text-warning text-sm font-semibold">{req.student_name.charAt(0)}</span></div>
                  <div className="min-w-0">
                    ...
                    {event && <p className="text-xs text-muted-foreground mt-0.5">Event: <span className="text-foreground">{event.title}</span></p>}
                    <p className="text-xs text-muted-foreground mt-0.5">Proof: {req.proof.slice(0, 60)}{req.proof.length > 60 ? '...' : ''}</p>
                  </div>
                </div>
                <span className={`campus-badge flex items-center gap-1 flex-shrink-0 ${statusBadge(req.status)}`}>{statusIcon(req.status)}<span className="capitalize">{req.status}</span></span>
              </div>
            </motion.div>
          );
        })}</div>}
            ...
            return (
              <div key={req.id} className="campus-card p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-full bg-muted flex items-center justify-center flex-shrink-0 text-xs font-semibold text-muted-foreground">{req.student_name.charAt(0)}</div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{req.student_name}{event ? ` — ${event.title}` : ''}</p>
                    <p className="text-xs text-muted-foreground">{req.roll_number} · {req.branch} · {req.created_at?.slice(0, 10)}</p>
                  </div>
                </div>
                <span className={`campus-badge flex items-center gap-1 flex-shrink-0 ${statusBadge(req.status)}`}>{statusIcon(req.status)}<span className="capitalize">{req.status}</span></span>
