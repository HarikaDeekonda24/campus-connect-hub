...
import { mockAttendanceRequests } from '@/lib/mock-data';
...
export default function AttendancePage() {
  ...
  const [form, setForm] = useState({ studentName: user?.name || '', rollNumber: user?.rollNumber || '', department: user?.department || '', eventName: '', eventDate: '', proof: '' });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">{isStudent ? 'Request attendance permission for events' : 'Review student attendance requests'}</p>
      </div>
        ...
        <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="campus-card p-6 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.studentName} onChange={e => update('studentName', e.target.value)} className="campus-input" readOnly />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">Roll Number</label>
              <input value={form.rollNumber} onChange={e => update('rollNumber', e.target.value)} className="campus-input" readOnly />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 block">Department</label>
              <input value={form.department} onChange={e => update('department', e.target.value)} className="campus-input" readOnly />
            </div>
            <div>
      ...
      {/* Request list */}
      <div>
        <h2 className="font-display text-foreground mb-3">{isStudent ? 'Your Requests' : 'All Requests'}</h2>
        <div className="space-y-3">
          {mockAttendanceRequests.map(req => (
            <div key={req.id} className="campus-card p-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{req.studentName} · {req.rollNumber} · {req.eventDate}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                {!isStudent && req.status === 'pending' && (
                  <div className="flex gap-1 ml-2">
                    <button onClick={() => toast.success('Approved!')} className="p-1.5 rounded bg-success/10 text-success hover:bg-success/20"><CheckCircle className="w-4 h-4" /></button>
